    ...
    while True:
        ...
        for x in range(BOARDSIZE):
            for y in range(BOARDSIZE):
                ...
                if mainBoard[x][y] == NOUGHT:
                    drawNought(x, y)
        if mainBoard[mouseboxx][mouseboxy] == BLANK:
            drawCross(mouseboxx, mouseboxy, LIGHTRED)
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                mouseboxx, mouseboxy = getBoxAtPixel(mousex, mousey)
...
def drawCross(x, y, colour):
    boxx, boxy = leftTopCoordsOfBox(x, y)
    pygame.draw.line(DISPLAYSURF, colour, (boxx + (BOXSIZE/10), boxy + (BOXSIZE/10)), (boxx + BOXSIZE - (BOXSIZE/10), boxy + BOXSIZE - (BOXSIZE/10)), 5)
    pygame.draw.line(DISPLAYSURF, colour, (boxx + BOXSIZE - (BOXSIZE/10), boxy + (BOXSIZE/10)), (boxx + (BOXSIZE/10), boxy + BOXSIZE - (BOXSIZE/10)), 5)
...
def gameOver(winner):
    ...
    if winner == 'nought' or winner == 'cross':
        label = myfont.render("The winner is the " + str(winner), 1, BLACK)
